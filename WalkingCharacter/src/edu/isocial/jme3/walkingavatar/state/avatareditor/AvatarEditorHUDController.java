...
import edu.isocial.jme3.walkingavatar.core.AvatarState;
...
//        scene.attachChild(ClothingItems.INSTANCE.getClothingItems().get(clothingItem));
    ...
    public void putOnClothingItem() {
        if(pants == true){
            AvatarState.INSTANCE.setPants(ClothingItems.INSTANCE.getClothingItems().get(clothingItemToWear));
            WalkingAvatarTest.getInstance().getAvatarEditor().updateAvatar();
        }else{
            AvatarState.INSTANCE.setShirt(ClothingItems.INSTANCE.getClothingItems().get(clothingItemToWear));
            WalkingAvatarTest.getInstance().getAvatarEditor().updateAvatar();
        }
    }
